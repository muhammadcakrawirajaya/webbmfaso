<div x-data="usePopper({ placement: 'right-end', offset: 12 })" @click.outside="isShowPopper && (isShowPopper = false)" class="flex">
    <button @click="isShowPopper = !isShowPopper" x-ref="popperRef" class="avatar size-12">
        ...
        <span
            class="{{  session('role') == 'team leader' ? 'absolute right-0 size-3.5 rounded-full border-2 border-white bg-secondary dark:border-navy-700' : 'absolute right-0 size-3.5 rounded-full border-2 border-white bg-success dark:border-navy-700' }}"></span>
    </button>
                    ...
                    <div>
                        <h2
                            ...
                            <img class="rounded-full w-5 h-5 mr-0.5 mb-1 inline-flex"
                                src="{{ asset('assets/images/logo-brand.png') }}" alt="avatar">Telkom
                        </h2>
                        <div class="text-xs text-slate-400 line-clamp-1 dark:text-navy-300">
                            Switch to Telkom
                        </div>
